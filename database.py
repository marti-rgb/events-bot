...
def save_event(event: dict) -> bool:
    ...
    try:
        ...
        c.execute('''
            INSERT INTO events (title, date, time, is_free, for_children, format, category_l1, category_l2, category_l1_arr, category_l2_arr, description, location, address, price, source_url, channel, city)
            VALUES (%(title)s, %(date)s, %(time)s, %(is_free)s, %(for_children)s, %(format)s, %(category_l1)s, %(category_l2)s, %(category_l1_arr)s, %(category_l2_arr)s, %(description)s, %(location)s, %(address)s, %(price)s, %(source_url)s, %(channel)s, %(city)s)
