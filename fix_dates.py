...
#
# Что делает:
# У части событий, сохранённых 26.08.2026 в 11:00 и позже, дата события
...
# Граница по времени сдвинута на 11:00 (момент, когда реально заработала
# Z.ai), плюс исключены уже вручную скрытые записи (is_ignored = false),
# чтобы не трогать их повторно.
SELECT_SQL = """
    SELECT id, source_url, date, channel
    FROM events
    WHERE created_at >= '2026-08-26 11:00'
      AND date < '2026-01-01'
